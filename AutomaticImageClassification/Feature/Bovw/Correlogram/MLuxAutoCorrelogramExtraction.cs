...
public class MLuxAutoCorrelogramExtraction : IAutoCorrelogramFeature {
    private MLuxMode mode;

    public MLuxAutoCorrelogramExtraction() {
        this.mode = MLuxMode.SuperFast;
    }

    public MLuxAutoCorrelogramExtraction(MLuxMode mode) {
        this.mode = mode;
    }
...
//        }
        ...
        float[][] correlogram = new float[maxFeatureValue][];
        for (int i = 0; i < maxFeatureValue; i++)
        {
            correlogram[i] = new float[distanceSet.Length];
        }
...
//        }
    ...
    private void getNumPixelsInNeighbourhood(int x, int y, int[][] quantPixels, int[] correlogramm, int maxFeatureValue, int[] distanceSet) {
        ...
        Array.Clear(correlogramm, 0, correlogramm.Length);
...
//            if (d > 1) System.arraycopy(correlogramm[d - 2], 0, correlogramm[d - 1], 0, correlogramm[d - 1].Length);
            ...
            if (this.mode == MLuxMode.QuarterNeighbourhood) {
            ...
            } else if (this.mode == MLuxMode.FullNeighbourhood) {
                //if (isInPicture(x + d, y + d, quantPixels.Length, quantPixels[0].Length))
    ...
    private static bool isInPicture(int x, int y, int maxX, int maxY) {
