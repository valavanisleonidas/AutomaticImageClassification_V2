    ...
    return function (a, b, c, d) {
        ...
        var v = t.CSS = {
            ...
            Lists: {
                ...
                transformsBase: ["translateX", "translateY", "scale", "scaleX", "scaleY", "rotate", "skewX", "skewY", "rotateZ"],
            ...
            Values: {
                ...
                getUnitType: function (a) {
                    return /^(rotate|skew)/i.test(a) ? "deg" : /(^(scale|scaleX|scaleY|rotate|scaleZ|alpha|flexGrow|flexHeight|zIndex|fontWeight)$)|((opacity|red|green|blue|alpha)$)/i.test(a) ? "" : "px"
                },
...
Vel = $ ? $.Velocity : Velocity, function (a) {
    a.fn.collapsible = function (b) {
		...
		this.each(function () {
			...
			b.accordion || "accordion" === i || void 0 === i ? (h = g.find("> li > .collapsible-header"), h.on("click.collapse", function (b) {
			    var d = a(b.target);
			    e(d) && (d = f(d)),
	...
	a.fn.dropdown = function (b) {
	    ...
	    this.each(function () {
	        ...
	        function e() {
	            d(),
				i.addClass("active"),
				h.constrain_width === !0 ? i.css("width", g.outerWidth()) : i.css("white-space", "nowrap");
	            var b = 0;
	            ...
	            if (j + i.innerWidth() > a(window).width() ? h.alignment = "right" : j - i.innerWidth() + g.innerWidth() < 0 && (h.alignment = "left"), "left" === h.alignment)
	                ...
	                e = 0,
					f = h.gutter,
					c = a(window).width() - g.position().left - g.innerWidth() + f,
					i.css({
					    right: c
					})
	            }
	        ...
	        if (d(), g.after(i), h.hover) {
	            ...
	            g.unbind("click." + g.attr("id")), g.bind("click." + g.attr("id"), function (b) {
					...
					i.hasClass("active") && a(document).bind("click." + i.attr("id"), function (b) {
...
(jQuery), function (a) {
    a(document).ready(function () {
		...
		a(document).on("input mousedown touchstart", g, function (b) {
		    ...
		    if (b.originalEvent.touches == null) return;
	...
	a.fn.material_select = function (b) {
	    a(this).each(function () {
	        if ($select = a(this), !$select.hasClass("browser-default")) {
	            ...
	            var f,
				g = a('<ul id="select-options-' + d + '" class="dropdown-content select-dropdown"></ul>'),
				h = $select.children("option");
	            f = void 0 !== $select.find("option:selected") ? $select.find("option:selected") : g.first(),
				h.each(function () {
				...
				k.before(i),
				a("body").append(g),
				$select.is(":disabled") || k.dropdown({
				    hover: !1
				}),
				$select.attr("tabindex") && a(k[0]).attr("tabindex", $select.attr("tabindex")),
				$select.addClass("initialized"),
				k.on("focus", function () {
				    a(this).trigger("open"),
					f = a(this).val(),
					selectedOption = g.find("li").filter(function () {
					    return a(this).text().toLowerCase() === f.toLowerCase()
					})[0],
					activateOption(g, selectedOption)
				}),
				k.on("blur", function () {
				...
				onKeyDown = function (b) {
				    ...
				    if (13 != b.which || g.is(":visible")) {
				        ...
				        })[0], newOption && activateOption(g, newOption)),
						13 == b.which && (activeOption = g.find("li.active:not(.disabled)")[0], activeOption && (a(activeOption).trigger("click"), k.trigger("close"))),
						40 == b.which && (newOption = g.find("li.active").next("li:not(.disabled)")[0], newOption && activateOption(g, newOption)),
						27 == b.which && k.trigger("close"),
						38 == b.which && (newOption = g.find("li.active").prev("li:not(.disabled)")[0], newOption && activateOption(g, newOption)),
						setTimeout(function () {
						    filterQuery = []
						}, 1e3)
				    }
        ...
        init: function (b) {
			...
			this.each(function () {
			    ...
			    function d(a) {
					...
					i != a && (e = h.eq(i), $caption = e.find(".caption"), e.removeClass("active"), e.velocity({
					    opacity: 0
					}, {
			    ...
			    if (-1 != i && (e = h.eq(i)), f.hasClass("fullscreen") || (f.height(b.indicators ? b.height + 40 : b.height), g.height(b.height)), h.find(".caption").each(function () {
						...
						a(this).css("background-image", "url(" + a(this).attr("src") + ")"),
						a(this).attr("src", "data:image/gif;base64,R0lGODlhAQABAIABAP///wAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==")
			    }), b.indicators) {
			    ...
			    e ? e.show() : (h.first().addClass("active").velocity({
			        ...
			        queue: !1,
			        easing: "easeOutQuad"
			    }), i = 0, e = h.eq(i), b.indicators && j.eq(i).addClass("active")),
...
(jQuery), function (a) {
    a(document).ready(function () {
		...
		a(document).on("mouseenter.fixedActionBtn", ".fixed-action-btn, .fixed-action-btn > a > .material-icons", function (event) {
		    if ($(event.target).hasClass("material-icons")) {
		        var c = a($(this).closest(".fixed-action-btn").get(0));
		        b(c)
		    }
		}),
		a(document).on("mouseleave.fixedActionBtn", ".fixed-action-btn", function (event) {
		    var b = a($(this).closest(".fixed-action-btn").get(0));
		    c(b)
		})
    }),
	a.fn.extend({
    ...
    var b = function (b) {
        if ($this = b, $this.hasClass("active") === !1) {
            ...
            if ($this.parent().hasClass("reversed")) {
                options = {
                    scaleY: "1",
                    rotate: "180deg"
                };
